...
import static org.apache.flume.sink.kfksyslog2json.S2JKafkaSinkConstants.BOOTSTRAP_SERVERS_CONFIG;
import static org.apache.flume.sink.kfksyslog2json.S2JKafkaSinkConstants.BATCH_SIZE;
import static org.apache.flume.sink.kfksyslog2json.S2JKafkaSinkConstants.DEFAULT_BATCH_SIZE;
import static org.apache.flume.sink.kfksyslog2json.S2JKafkaSinkConstants.BROKER_LIST_FLUME_KEY;
import static org.apache.flume.sink.kfksyslog2json.S2JKafkaSinkConstants.DEFAULT_ACKS;
import static org.apache.flume.sink.kfksyslog2json.S2JKafkaSinkConstants.DEFAULT_KEY_SERIALIZER;
import static org.apache.flume.sink.kfksyslog2json.S2JKafkaSinkConstants.DEFAULT_TOPIC;
import static org.apache.flume.sink.kfksyslog2json.S2JKafkaSinkConstants.DEFAULT_VALUE_SERIAIZER;
import static org.apache.flume.sink.kfksyslog2json.S2JKafkaSinkConstants.KAFKA_PRODUCER_PREFIX;
import static org.apache.flume.sink.kfksyslog2json.S2JKafkaSinkConstants.KEY_HEADER;
import static org.apache.flume.sink.kfksyslog2json.S2JKafkaSinkConstants.OLD_BATCH_SIZE;
import static org.apache.flume.sink.kfksyslog2json.S2JKafkaSinkConstants.REQUIRED_ACKS_FLUME_KEY;
import static org.apache.flume.sink.kfksyslog2json.S2JKafkaSinkConstants.TOPIC_CONFIG;
import static org.apache.flume.sink.kfksyslog2json.S2JKafkaSinkConstants.TOPIC_HEADER;
import static org.apache.flume.sink.kfksyslog2json.S2JKafkaSinkConstants.KEY_SERIALIZER_KEY;
import static org.apache.flume.sink.kfksyslog2json.S2JKafkaSinkConstants.MESSAGE_SERIALIZER_KEY;
import static org.apache.flume.sink.kfksyslog2json.S2JKafkaSinkConstants.SYSLOG2JSON;
import static org.apache.flume.sink.kfksyslog2json.S2JKafkaSinkConstants.DEFAULT_SYSLOG2JSON;
 ...
 * key
 */
public class S2JKafkaSink extends AbstractSink implements Configurable {

  private static final Logger logger = LoggerFactory.getLogger(S2JKafkaSink.class);
        ...
        try {
          ...
          if (partitionId != null) {
            if (useSyslog2Json == false) {
              ...
              record = new ProducerRecord<String, byte[]>(eventTopic, partitionId, eventKey,
                      S2JEventHelper.dumpEvent(event, maxBytesToLog).getBytes());
            }
          } else {
            if (useSyslog2Json == false) {
  ...
  public void configure(Context context) {
    ...
    useSyslog2Json = context.getBoolean(S2JKafkaSinkConstants.SYSLOG2JSON,
                                        S2JKafkaSinkConstants.DEFAULT_SYSLOG2JSON);
    ...
    useAvroEventFormat = context.getBoolean(S2JKafkaSinkConstants.AVRO_EVENT,
                                            S2JKafkaSinkConstants.DEFAULT_AVRO_EVENT);

    partitionHeader = context.getString(S2JKafkaSinkConstants.PARTITION_HEADER_NAME);
    staticPartitionId = context.getInteger(S2JKafkaSinkConstants.STATIC_PARTITION_CONF);

    if (logger.isDebugEnabled()) {
      logger.debug(S2JKafkaSinkConstants.AVRO_EVENT + " set to: {}", useAvroEventFormat);
    }
  ...
  private void translateOldProps(Context ctx) {
    ...
    if (!(ctx.containsKey(KAFKA_PRODUCER_PREFIX + ProducerConfig.ACKS_CONFIG))) {
      String requiredKey = ctx.getString(
              S2JKafkaSinkConstants.REQUIRED_ACKS_FLUME_KEY);
